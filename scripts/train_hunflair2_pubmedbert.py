...
def main():
    ...
    parser.add_argument(
        "--device",
        type=str,
        default="cuda" if flair.device.type == "cuda" else "cpu",
        choices=["cpu", "cuda"],
    )
    ...
    if args.use_safetensors:
        emb_kwargs["use_safetensors"] = True

    embeddings = TransformerWordEmbeddings(**emb_kwargs)

    # Manually set tokenizer max length (Flair wraps tokenizer internally)
    try:
